...
def main():
    ...
    check_for_updates()
    print("--------------------------------------------------------------------------------------------------")
    print("🚀 Netflix Checker - PURE TELEGRAM BOT MODE (main checking logic untouched)")
    print("")
    ...
    # Get bot token from environment variable (recommended) or you can hardcode for testing
    bot_token = os.environ.get("TELEGRAM_BOT_TOKEN", "8542796779:AAE9DIadsPS_UPmlvFFO0qdifsMtydV_P-8").strip()

    if not bot_token:
        print("❌ TELEGRAM_BOT_TOKEN environment variable not set!")
        print("   1. Create a bot with @BotFather on Telegram")
        print("   2. Get the token")
        print("   3. Set it: export TELEGRAM_BOT_TOKEN='123456:ABC-DEF...'  (Linux/Mac)")
        print("      or set in your system environment variables (Windows)")
        print("   4. Restart this script")
        return

    print("✅ Bot token loaded. Starting Telegram bot...")
    print("   Users can now interact with your bot on Telegram.")
    print("   Press Ctrl+C to stop.\n")

    bot = telebot.TeleBot(bot_token, parse_mode="HTML")

    # State for single-user bot (simple approach)
    bot_state = {
        "checking": False,
        "last_chat_id": None,
    }
    ...
    def send_welcome(message):
        ...
        welcome_text = (
            ...
            "This is a <b>pure Telegram interface</b> for the powerful Netflix checker.\n"
            "All core checking, NFToken, extraction, and output logic is <b>untouched</b>.\n\n"
            "<b>📋 How to use:</b>\n"
            "1️⃣ Upload your cookie files (.txt or .json) — you can send multiple\n"
            "2️⃣ (Optional) Upload <code>proxy.txt</code> to use proxies\n"
            "3️⃣ Send /check to start the full checking process\n"
            "4️⃣ When finished, you will receive a ZIP with all results (same structure as CLI)\n\n"
            "<b>🛠️ Commands:</b>\n"
            "/check — Start checking cookies in the cookies/ folder\n"
            "/clear — Clear all files from cookies/ folder (start fresh)\n"
            "/status — Show current cookies count and if a check is running\n"
            "/help — Show this message\n\n"
            "💡 <i>Live progress &amp; detailed logs appear in the terminal where you started the bot.</i>\n"
            "💡 Results are also saved in output/run_YYYY-MM-DD_HH-MM-SS/ exactly like the original CLI.\n\n"
            "Made with ❤️ — core by harshitkamboj | TG wrapper added"
        )
    ...
    def status_cmd(message):
        ...
        try:
            ...
            count = len(cookie_files)
            checking_status = "🔄 <b>RUNNING</b>" if bot_state["checking"] else "✅ <b>IDLE</b>"
            status_text = (
                f"📊 <b>Bot Status</b>\n\n"
                f"• Cookies in folder: <b>{count}</b>\n"
                f"• Check in progress: {checking_status}\n"
                f"• Last run output: Check the <code>output/</code> folder or wait for ZIP after /check\n\n"
                f"Use /check to start processing."
            )
            bot.send_message(chat_id, status_text)
        except Exception as e:
            bot.send_message(chat_id, f"Error getting status: {e}")
    ...
    def clear_cmd(message):
        ...
        if bot_state["checking"]:
            bot.send_message(chat_id, "⚠️ Cannot clear while a check is running. Please wait.")
            return
        try:
            for filename in os.listdir(cookies_folder):
                ...
                try:
                    ...
                    pass
            bot.send_message(chat_id, "🗑️ <b>Cookies folder cleared successfully!</b>\nYou can now upload fresh cookie files.")
        except Exception as e:
            bot.send_message(chat_id, f"Error clearing folder: {e}")
    ...
    def handle_document(message):
        ...
        if bot_state["checking"]:
            bot.send_message(chat_id, "⚠️ A check is currently running. Please wait until it finishes before uploading more files.")
            return
    ...
    def fallback(message):
        bot.send_message(
            message.chat.id,
            "Unknown command. Send /help to see available commands or upload a cookie file."
        )
